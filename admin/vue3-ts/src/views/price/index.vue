<template>
  <div class="price-container">
    <el-tabs class="head-tabs" style="overflow:hidden;" v-model="activeName" type="border-card"
             @tab-click="handleClick">
      <el-tab-pane label="Shopee" name="shopee">
        <div class="fright">
          <el-button size="small" type="primary" @click="syncAllItem">同步全部产品</el-button>
          <el-button size="small" type="primary" @click="syncAllItem">同步调价中的产品</el-button>
        </div>
        <span class="search-label">店铺：</span>
        <el-radio-group v-model="searchParams.shopId" size="small" @change="shopChange">
          <el-radio-button label="">全部</el-radio-button>
          <el-radio-button v-for="shop in shops" :key="shop.id" :label="shop.id">{{
              shop.name
            }}
          </el-radio-button>
        </el-radio-group>
        <div class="hy-mt-20">
          <span class="search-label">状态：</span>
          <el-radio-group v-model="searchParams.pricingType" size="small" @change="statusChange">
            <el-radio-button label="">全部</el-radio-button>
            <el-radio-button :label="0">未调价</el-radio-button>
            <el-radio-button :label="2">限量调价</el-radio-button>
            <el-radio-button :label="1">自动调价</el-radio-button>
          </el-radio-group>
        </div>
        <div class="hy-mt-20">
          <span class="search-label">展示：</span>
          <el-radio-group v-model="showStatus" size="small">
            <el-radio-button label="">全部SKU</el-radio-button>
            <el-radio-button label="1">只展示调价中的SKU</el-radio-button>
          </el-radio-group>
        </div>
        <el-table
            :data="tableData"
            size="small"
            class="table hy-mt-20"
            style="width: 100%">
          <el-table-column type="index" align="center" label="序号"></el-table-column>
          <el-table-column prop="date" align="center" label="图片" min-width="150">
            <template v-slot="{ row }">
              <el-tooltip placement="right-end" effect="light">
                <template #content>
                  <img class="pic-large" :src="row.image"/>
                </template>
                <img class="pic" :src="row.image"/>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column prop="itemName" align="left" label="标题/产品ID" min-width="120">
            <template v-slot="{ row }">
              <p class="fs-12">{{ row.itemName }}</p>
              <br/>
              <p class="color-hy-blue">{{ row.itemId }}</p>
            </template>
          </el-table-column>
          <el-table-column prop="name" align="left" label="SKU" min-width="900">
            <template #header>
              <div style="display: flex;justify-content: space-evenly">
                <span class="item-title">SKU</span>
                <span class="item-normal l1">未调价格</span>
                <span class="item-normal">调价状态</span>
                <span class="item-normal">调后价格</span>
                <span class="item-normal l4">调价开始时间</span>
                <span class="item-normal l5">调价要求/销售记录（此列中的价格为调后价格）</span>
                <span class="item-normal l6">操作</span>
              </div>
            </template>
            <template v-slot="{ row, $index }">
              <div class="bd-bm-da-d item-box" v-for="(item, idx) in row.modelList"
                   :key="item.modelId"
                   v-show="(showStatus === '' && (row.spread || idx < DEFAULT_SKU_SHOW_COUNT)) || (showStatus !== '' && item.pricingType !== 0 && (row.spread || idx <= row.maxPricingIndex))">
                <p class="item-title">
                  {{ item.modelSku }}
                  <br/>
                  <span class="color-hy-blue">{{ item.modelId }}</span>
                </p>
                <p class="item-normal l1">{{ item.currentPrice }}</p>
                <p class="item-normal">{{ skuStatusMap[item.pricingType] }}</p>
                <p class="item-normal">{{ item.promotionPrice || '--' }}</p>
                <p class="item-normal l4 fs-12">{{ item.startTime || '--' }}</p>
                <div class="item-normal l5">
                  <div>
                    <!-- 懒加载获取销售记录 -->
                    <lazy-component :key="item.key">
                      <Records :item="item" :row="row" :skuIndex="idx"
                               :setOrderList="setOrderList"/>
                    </lazy-component>
                    <div v-if="item.pricingType === 2 && item.orderTitle.length" class="bd-bm-da-d">
                      限量调价：
                      <span v-for="(title, i) in item.orderTitle" :key="i">价格:{{
                          title.modelPromotionPrice
                        }}，{{ title.modelPromotionStock }}件；</span>
                    </div>
                    <!--                    <div v-if="item.pricingType === 1">-->
                    <!--                      <p class="bd-bm-da-d">自动调价：<span>最低价:40；</span><span>变动:0.01元</span>-->
                    <!--                      </p>-->
                    <!--                      <span class="color-hy-blue">链接1:20；</span><span-->
                    <!--                      class="color-hy-blue">链接2:80；</span>-->
                    <!--                    </div>-->
                    <div v-if="!item.orderTitle.length">--</div>
                    <div>
                      <span
                          v-for="(order, idx2) in item.orderList.slice(0,DEFAULT_ORDER_SHOW_COUNT)" :key="order.orderSn">
                        <br v-if="idx2 !== 0"/>
                        <span>{{ order.createTime }} {{ order.modelQuantityPurchased }}件  价格:{{ order.modelDiscountedPrice }}；</span>
                      </span>
                      <!--                      <span>2020-01-25 12:36:15   8件   价格:30；</span>-->
                      <el-popover
                          placement="bottom"
                          title="销售记录"
                          width="300"
                          trigger="click">
                        <span v-for="(order, idx2) in item.orderList" :key="order.orderSn">
                        <br v-if="idx2 !== 0"/>
                        <p>{{ order.createTime }} {{ order.modelQuantityPurchased }}件 价格:{{ order.modelDiscountedPrice }}；</p>
                      </span>
                        <template #reference>
                          <el-button v-show="item.orderList.length > DEFAULT_ORDER_SHOW_COUNT" class="hy-ml-10" icon="el-icon-arrow-down" type="text" size="small">
                            更多
                          </el-button>
                        </template>
                      </el-popover>
                    </div>
                  </div>
                </div>
                <p class="item-normal l6">
                  <el-button type="text" size="mini" @click="() => limitAdjust(row, item)">限量调价</el-button>
                  <br/>
                  <el-button type="text" size="mini" @click="() => autoAdjust(row, item)">自动调价</el-button>
                  <br/>
                  <el-button :disabled="item.pricingType === 0" type="text" size="mini" @click="() => cancelAdjust(row, item)">取消调价
                  </el-button>
                </p>
              </div>
              <el-button
                  v-if="(showStatus === '' && row.modelList.length > DEFAULT_SKU_SHOW_COUNT) || (showStatus !== '' && row.totalPricing > DEFAULT_SKU_SHOW_COUNT)"
                  class="fright" :icon="row.icon" type="text" size="small"
                  @click="() => showMoreOrLess(row, $index)">
                {{ row.showText }}
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
            class="fright hy-mt-15"
            background
            :current-page="searchParams.index"
            :page-count="searchParams.pageSize"
            @size-change="(pageSize)=>getTableData({pageSize})"
            @current-change="(index) => getTableData({index})"
            layout="sizes, prev, pager, next, total"
            :page-sizes="[20, 50, 100]"
            :total="total">
        </el-pagination>
        <el-dialog
            title="限量调价"
            class="adjust-dialog"
            v-model="dialogVisible"
            width="50%">
          <!--          <p class="title">基本信息</p>-->
          <!--          <el-form class="dialog-form" label-position="right" label-width="80px"-->
          <!--                   :model="form">-->
          <!--            <el-form-item label="活动名称">-->
          <!--              <el-input size="mini" v-model="form.discountName"></el-input>-->
          <!--            </el-form-item>-->
          <!--            <el-form-item label="开始时间">-->
          <!--              <el-input size="mini" v-model="form.startTime"></el-input>-->
          <!--            </el-form-item>-->
          <!--            <el-form-item label="结束时间">-->
          <!--              <el-input size="mini" v-model="form.endTime"></el-input>-->
          <!--            </el-form-item>-->
          <!--          </el-form>-->
          <el-table
              :data="dialogTableData"
              style="width: 100%">
            <el-table-column prop="current_price" align="center" label="价格"></el-table-column>
            <el-table-column align="center" label="调后价格" min-width="100">
              <template v-slot="{ row }">
                <el-input size="small" v-model.trim="row.model_promotion_price"></el-input>
              </template>
            </el-table-column>
            <el-table-column align="center" label="件数" min-width="100">
              <template v-slot="{ row }">
                <el-input size="small" v-model.trim="row.model_promotion_stock"></el-input>
              </template>
            </el-table-column>
          </el-table>
          <template #footer>
            <span class="dialog-footer">
            <el-button size="small" @click="dialogVisible = false">取 消</el-button>
            <el-button size="small" type="primary" @click="limitAdjustConfirm">确 定</el-button>
          </span>
          </template>
        </el-dialog>
        <el-dialog
            title="自动调价"
            class="adjust-dialog2"
            top="6vh"
            v-model="dialogVisible2"
            width="50%">
          <!--          <div style="display: flex; justify-content: space-evenly; align-items: center">-->
          <!--            <span class="hy-mr-20" style="flex: 50px 1">价格：{{ curSku.currentPrice }}</span>-->
          <el-form inline :model="autoAdjustForm" :rules="rules" ref="autoAdjustFormRef">
            <el-form-item label="价格：">
              <span class="hy-mr-40">{{ curSku.currentPrice }}</span>
            </el-form-item>
            <el-form-item label="最低价：" prop="price">
              <el-input v-model.trim="autoAdjustForm.price" size="small"></el-input>
            </el-form-item>
            <el-form-item label="变动：" prop="gap">
              <el-input v-model.trim="autoAdjustForm.gap" size="small"></el-input>
            </el-form-item>
          </el-form>
          <!--            <span style="flex: 100px 3">-->
          <!--              最低价：<el-input size="small" style="width: 80%"></el-input>-->
          <!--            </span>-->
          <!--            <span class="hy-ml-10" style="flex: 100px 3">-->
          <!--              变动：<el-input size="small" style="width: 80%;"></el-input>-->
          <!--            </span>-->
          <!--          </div>-->
          <el-table
              :data="dialogTableData2"
              style="width: 100%">
            <el-table-column prop="name" align="center" label="名称" min-width="50">
              <template v-slot>
                <el-input size="small"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="url" align="center" label="链接" min-width="100">
              <template v-slot>
                <el-input size="small"></el-input>
              </template>
            </el-table-column>
          </el-table>
          <template #footer>
            <span class="dialog-footer">
            <el-button size="small" @click="dialogVisible2 = false">取 消</el-button>
            <el-button size="small" type="primary" @click="autoAdjustConfirm">确 定</el-button>
          </span>
          </template>
        </el-dialog>
      </el-tab-pane>
      <el-tab-pane label="Lazada" name="lazada"></el-tab-pane>
    </el-tabs>
  </div>
</template>

<script lang="ts">
import {
  CANCEL_PRICEING_URL,
  GET_ITEM_INFO_URL,
  GET_SHOP_INFO_URL, LIMIT_PRICEING_URL,
  REFRESH_SHOPEE_TOKEN_URL, SYNC_ITEM_DATA_URL
} from '@/http/urls';
import {formatDate, getLoading} from '@/common/utils';
import Records from '@/views/authorize/Records.vue';
import {onMounted, reactive, Ref, ref, watch} from "vue";
import {ElMessage, ElMessageBox} from "element-plus";
import http from "@/http";
import {LoadingReturnValue} from "@/types/ext-types";
import { LooseObject } from '@/types';

export default {
  name: 'index',
  components: {Records},
  setup(): unknown {
    const activeName = ref('shopee'),
        dialogVisible = ref(false),
        dialogVisible2 = ref(false),
        form = reactive({
          discountName: '',
          startTime: '',
          endTime: '',
        }),
        dialogTableData = reactive([{}, {}, {}, {}, {}]),
        dialogTableData2 = reactive([{}, {}, {}, {}, {}, {}, {}, {}, {}, {}]),
        tableData = reactive([]),
        total = ref(0),
        showStatus = ref(''), // 展示sku状态，''全部，1只展示调价中的sku
        searchParams = reactive({
          shopId: 0,
          pageSize: 20,
          index: 1,
          itemStatus: 1,
          pricingType: '',
          token: ''
        }),
        curRow: Ref = reactive({}) as Ref, // 当前操作的产品
        curSku: Ref = reactive({}) as Ref, // 当前操作产品的sku
        shops = reactive([]),
        tokens = reactive([]),
        DEFAULT_SKU_SHOW_COUNT = 5, // 默认展示单个产品sku最多数量
        shopIdTokenMap: LooseObject = reactive({}),
        skuStatusMap = reactive({
          0: '未调价',
          1: '自动调价',
          2: '限量调价'
        }),
        spreads = {
          more: false,
          less: true
        },
        showTexts = {
          more: '展开',
          less: '收起'
        },
        showIcons = {
          more: 'el-icon-arrow-down',
          less: 'el-icon-arrow-up'
        },
        DEFAULT_ORDER_SHOW_COUNT = 2, // 默认展示单个产品sku最多销售记录
        autoAdjustForm = reactive({
          price: '',
          gap: ''
        }),
        rules = {
          price: [
            {required: true, message: '请填写最低价', trigger: 'blur'}
          ],
          gap: [
            {required: true, message: '请填写变动价格', trigger: 'blur'}
          ]
        },
        autoAdjustFormRef = ref()
    let loadingVM: LoadingReturnValue = {} as LoadingReturnValue

    // 自动调价确定
    function autoAdjustConfirm() {
      autoAdjustFormRef.value.validate((valid: boolean) => {
        if (!valid) return;

      });
    }

    // 设置销售记录
    function setOrderList(data: LooseObject, row: LooseObject, item: LooseObject) {
      if (data.content?.length || data.title?.length) {
        item.orderList = data.content || [];
        item.orderTitle = data.title || [];
        item.key = item.modelId;
        // this.$set(row.modelList, skuIndex, item);
      }
    }

    // 限量调价确定
    function limitAdjustConfirm() {
      // const {curRow: {shopId, itemId}, curSku: {modelId}} = this,
      const {shopId, itemId} = curRow.value, {modelId} = curSku.value,
          modelList = dialogTableData.filter((i: LooseObject) => i.model_promotion_price && i.model_promotion_stock), // 过滤不完整数据
          params = {
            shopId,
            token: (shopIdTokenMap as LooseObject)[shopId],
            itemId,
            type: 2,
            modelList
          };
      if (!modelList.length) return ElMessage.error('请至少有效填写一行数据');
      if (modelList.some((i: LooseObject) => !(/^\d[\d,.]*/).test(i.model_promotion_price) || !(/^\d+$/).test(i.model_promotion_stock))) return ElMessage.error('请正确填写输入的值');
      modelList.forEach((i: LooseObject) => i.model_id = modelId);
      http.post(LIMIT_PRICEING_URL, params).then((r: LooseObject) => {
        if (r.code === 1) {
          ElMessage.success(r.msg);
          dialogVisible.value = false;
          getTableData();
        } else {
          ElMessage.error(r.msg);
        }
      });
    }

    // 同步全部产品
    function syncAllItem() {
      loadingVM = getLoading(true);
      http.post(SYNC_ITEM_DATA_URL, {
        shopId: searchParams.shopId,
        token: searchParams.token
      }).then((r: any) => {
        let total = r * 2, i = 1, timer = setInterval(() => {
          loadingVM.setText(`加载中，时间可能较长，请耐心等待（${(i / total * 100).toFixed(0)}%）`);
          if (++i === total) {
            getTableData();
            clearInterval(timer);
          }
        }, 500);
      });
    }

    // sku展开或收起
    function showMoreOrLess(row: LooseObject) {
      const key = row.showText === showTexts.more ? 'less' : 'more';
      row.showText = showTexts[key];
      row.spread = spreads[key];
      row.icon = showIcons[key];
      // this.$set(this.tableData, index, row);
    }

    function getTableData(params = {}) { // 参数覆盖，同步更新记录中的searchParams
      if (searchParams.shopId === 0) return;
      http.post(GET_ITEM_INFO_URL, Object.assign(searchParams, params)).then((r: LooseObject) => {
        loadingVM.close();
        tableData.values = r.data;
        total.value = r.total;
        tableData.forEach((i: LooseObject) => {
          i.maxPricingIndex = i.modelList.length - 1;
          i.showText = showTexts.more;
          i.icon = showIcons.more;
          let count = 0;
          i.modelList.forEach((model: LooseObject, idx: number) => {
            if (model.pricingType !== 0 && ++count === DEFAULT_SKU_SHOW_COUNT) i.maxPricingIndex = idx; // 设置只展示调价的sku时最大显示的索引
            model.orderList = [];
            model.orderTitle = [];
            model.key = Math.random();
          });
          i.totalPricing = count; // 单个产品调价中的sku总数
        });
      });
    }

    // 切换状态
    function statusChange(pricingType: number | string) {
      resetSearch();
      getTableData({pricingType});
    }

    // 重置搜索条件
    function resetSearch() {
      searchParams.pageSize = 20;
      searchParams.index = 1;
    }

    // 切换店铺
    function shopChange(shopId: number | string) { // ''表示全部店铺
      resetSearch();
      getTableData({shopId, token: shopIdTokenMap[shopId]});
    }

    // 获取店铺信息
    function getShopData() {
      http.post(GET_SHOP_INFO_URL).then(r => {
        shops.values = r.data.map((i: LooseObject) => ({
          name: i.shopName,
          id: i.shopId
        }));
        searchParams.shopId = shops.length ? (shops[0] as any).id : 0; // 默认选第一个店铺
        http.post(REFRESH_SHOPEE_TOKEN_URL).then(r => { // watch监听tokens同步tokenMap
          setInterval(() => { // 3小时刷新一次
            http.post(REFRESH_SHOPEE_TOKEN_URL).then(r => tokens.values = r.data);
          }, 10800000);
          tokens.values = r.data;
          searchParams.token = r.data[0].token;
          getTableData();
        });
      });
    }

    console.log(curRow)

    // 更新当前行操作的对象
    function setCurRow(row: any, sku: any) {
      curRow.value = row; // todo
      curSku.value = sku;
    }

    // 点击限量调价
    function limitAdjust(row: any, sku: any) {
      setCurRow(row, sku);
      dialogVisible.value = true;
      dialogTableData.forEach((i: any) => {
        i.current_price = sku.currentPrice;
      });
      // $set(this, 'dialogTableData', dialogTableData);
      // window.open(location.origin + APP_BASE_PATH + EDIT_LIMIT_PRICE_PATH + `?itemId=${row.itemId}`);
    }

    // 点击自动调价
    function autoAdjust(row: any, sku: any) {
      setCurRow(row, sku);
      dialogVisible2.value = true;
    }

    // 取消调价
    function cancelAdjust(row: any, sku: any) {
      setCurRow(row, sku);
      ElMessageBox.confirm('确定取消调价?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        http.post(CANCEL_PRICEING_URL, {
          shopId: curRow.value.shopId,
          token: shopIdTokenMap[curRow.value.shopId],
          modelId: curSku.value.modelId
        }).then((r: any) => {
          if (r.code === 1) {
            ElMessage.success(r.msg);
          } else {
            ElMessage.error(r.msg);
          }
          getTableData();
        });
      });
    }

    function handleClick() {
      console.log('handle click')
    }

    watch(tokens, v => {
      shopIdTokenMap.value = {};
      v.forEach((i: any) => shopIdTokenMap[i.shopId] = i.token);
    })
    onMounted(() => {
      loadingVM = getLoading(true);
      getShopData();
    })
    return {
      activeName,
      dialogVisible,
      dialogVisible2,
      form,
      dialogTableData,
      dialogTableData2,
      tableData,
      total,
      showStatus,
      searchParams,
      curRow,
      curSku,
      shops,
      loadingVM,
      tokens,
      DEFAULT_SKU_SHOW_COUNT,
      shopIdTokenMap,
      skuStatusMap,
      spreads,
      showTexts,
      showIcons,
      DEFAULT_ORDER_SHOW_COUNT,
      autoAdjustForm,
      rules,

      formatDate,
      autoAdjustConfirm,
      setOrderList,
      limitAdjustConfirm,
      syncAllItem,
      showMoreOrLess,
      statusChange,
      shopChange,
      limitAdjust,
      autoAdjust,
      cancelAdjust,
      handleClick,
    };
  }
};
</script>

<style lang="scss" scoped>
.price-container {
  margin-left: 20px;
  box-shadow: 0 0 5px #ccc;

  .search-label {
    font-size: 13px;
  }

  .head-tabs {
    border: none;
    height: 100%;

    ::v-deep .el-tabs__content {
      padding-left: 20px;
    }
  }

  .table {
    border-bottom: none;

    .pic {
      display: block;
      width: 150px;
      height: 150px;
      margin: 0 auto;
    }

    .item-box {
      display: flex;
      justify-content: space-evenly;
      align-items: center;
      line-height: 2.5;
    }

    .item-title, .item-normal {
      display: inline-block;
    }

    .item-title {
      width: 80px;
    }

    .item-normal {
      flex: 60px 1;
      text-align: center;

      &.l1 {
        flex-basis: 50px;
      }

      &.l4 {
        flex-basis: 75px;
      }

      &.l5 {
        display: flex;
        justify-content: center;
        flex: 350px 8;

        & > div {
          margin: 0 auto;
          text-align: left;
        }
      }

      &.l6 {
        flex-basis: 50px;
      }
    }
  }

}

.adjust-dialog {

  .title {
    border-bottom: 1px solid #ccc;
  }
}
</style>

<style lang="scss">
.table td.el-table__cell {
  border-bottom-color: #ccc !important;
}

.adjust-dialog, .adjust-dialog2 {
  > div {
    min-width: 750px;
  }
}

.pic-large {
  display: block;
  width: 300px;
  height: 300px;
  margin: 0 auto;
}

.dialog-form {
  //.el-form-item {
  //  width: 24%;
  //
  //  .el-form-item__content {
  //    width: calc(100% - 80px);
  //  }
  //}
}
</style>
