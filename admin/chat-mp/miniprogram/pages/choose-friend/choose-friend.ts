...
Page({
  ...
  getRealContent(msg: SgMsg | GpMsg) {
    return msg.status === MsgStatus.reply ? (msg.content as ReplyContent).data : msg.content
  },
  async confirm() { // todo 测试
    ...
    for (const {to, chatType, avatar} of selecteds) {
      ...
      if (isSingleTran) { // 逐条转发
        data.forEach((msg: SgMsg | GpMsg, i: number) => msgs.push({content: this.getRealContent(msg), type: msg.type, fakeId: fakeId + i}))
