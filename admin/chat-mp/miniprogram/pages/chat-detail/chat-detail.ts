...
Page({
  ...
  onReady() {
    ...
    iac.onPause(() => { this.setData({ audioPlayState: 0 }) })
    iac.onPlay(() => { this.setData({ audioPlayState: 1 }) })
    iac.onError((res) => {
  ...
  loadInitMessage() {
    ...
    if (messageInfo.messages && messageInfo.messages.length >= LOAD_MESSAGE_COUNT) { // 如果全局有足够的消息数量则直接从内存中取
      ...
      if (index) { // 有本地消息
        messageInfo.loadedMessagesMinIndex = messageInfo.maxMessagesIndex = messageInfo.maxMessagesIndex
  ...
  handleViewMessageTime(target?: Message[], start = 0, end?: number) {
    ...
    for (let i = start + 1; i < end!; i++) {
      ...
      if (this.cmpTime(target[i - 1].createdAt!, createdAt)) {
        res.push({
          data: formatSimpleDate(new Date(createdAt)),
          type: 0,
        })
      }
      res.push(target[i])
    ...
    if (start === 0) {
      res.unshift({
        data: formatSimpleDate(new Date(target[0].createdAt!)),
        type: 0
      })
    }
    return res
  ...
  async send() {
    ...
    const type = this.data.recordState === 2 && this.data.inputState === 1 ? 3 : 1 // 语音或文字
