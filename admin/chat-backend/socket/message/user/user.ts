...
export async function searchUsers(ws: ExtWebSocket, session: SessionData, data: RequestMessage<SearchUserQuery>) {
  ...
  const {result} = await getUserByUsername(data.data.username)
...
export async function addUser(ws: ExtWebSocket, session: SessionData, data: RequestMessage<AddUserBody>) {
  ...
  const {result: users} = await getUserByUsername(to)
  ...
  if (!result.length) {
    const {result: {insertId}} = await addFriendApl(to, from, reason)
    ...
    await resetFriendAplById(friendAplId, reason)
  }
  ...
  if (result2.length) {
    ...
    await resetContactById(contactId, remark)
  } else {
