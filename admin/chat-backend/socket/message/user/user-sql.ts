...
export function selectFriendAplByAddUser(to: Users.Username, from: Users.Username) {
  return executeSocketSql<{ status: FriendApls.Status }[]>('select status from friend_applications where `from` = ? and `to` = ?;', [from, to])
}

export function addFriendApl(to: Users.Username, from: Users.Username, reason: string) {
  return executeSocketSql<InsertModal>('insert friend_applications(`from`, `to`, reason, status) values(?, ?, ?, 0);', [from, to, reason])
}

export function resetFriendApl(to: Users.Username, from: Users.Username, reason: string) {
  return executeSocketSql('update friend_applications set status = 0, reason = ? where `from` = ? and `to` = ?;', [reason, from, to])
}

export function updateFriendAplStatus(to: Users.Username, from: Users.Username, status: number) {
