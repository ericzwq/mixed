...
export const executeSocketSql: CustomSocketQueryFunction = function(options, values?, sqlConfig?: SqlConfig) {
	...
	return new Promise((resolve, reject) => {
		pool.getConnection(async (err: MysqlError, connection: PoolConnection) => {
			...
			const query = connection.query(options, values, (error: MysqlError | null, result: any, fields: FieldInfo[] | undefined) => {
				if (error) {
					console.log(error)
					if (sqlConfig!.errorRollback) connection.query('rollback')
					reject({ query, error, fields })
				} else {
					resolve({ query, result, fields })
				}
